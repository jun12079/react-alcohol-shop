...
function OrderModal({ tempOrder, isOrderModalOpen, setIsOrderModalOpen, getOrders }) {
  ...
  const updateOrderItemQty = (productItem_id, price, qty) => {
    ...
    // 判斷是否有優惠券，如果有優惠券，用折扣後的價格。沒有優惠券，則價格不變
    const couponPercent = obj[productItem_id].coupon ? obj[productItem_id].coupon.percent * 0.01 : 1;
    const discountedPrice = price * couponPercent;

    obj[productItem_id].qty = qty;
    obj[productItem_id].total = qty * discountedPrice;
    obj[productItem_id].final_total = qty * discountedPrice;

    // 重新加總每個品項的final_total = 新的總金額
    const newTotal = Object.values(obj).reduce((sum, item) => sum + item.final_total, 0);
                  ...
                  <table className="table align-middle">
                    ...
                    <tbody>
                      {Object.values(modalData.products).map((productItem) => {
                        return (
                          <tr key={productItem.id}>
                            ...
                            <td className="text-end">{Math.round(productItem.coupon ? productItem.product.price * productItem.coupon.percent * 0.01 : productItem.product.price)}</td>
                            <td className="text-end">{Math.round(productItem.final_total)}</td>
                            <td>
                              <button
                                onClick={() => {
                      ...
                      <tr>
                        <td></td>
                        <td></td>
                        <td className="text-end fw-bold">總計 : </td>
                        <td className="text-end">{modalData.total}</td>
                        <td></td>
                      </tr>
                    </tbody>
                  </table>
