...
export default function CartPage() {
  ...
  const updateCartItem = async (cartItem_id, product_id, qty) => {
    ...
    try {
      ...
      dispatch(pushMessage({ text: "更新購物車商品失敗", status: 'failed' }))
    } finally {
      setIsScreenLoading(false);
    }
  }
  ...
  const removeCartItem = async (id) => {
    ...
    try {
      ...
      dispatch(pushMessage({ text: "刪除購物車項目失敗", status: 'failed' }))
    } finally {
      setIsScreenLoading(false);
    }
  }
  ...
  const handleCoupon = async (couponCode) => {
    ...
    try {
      ...
      if (res.data.success === false) {
        dispatch(pushMessage({ text: res.data.message, status: 'failed' }))
        setCouponCode("");
        return;
      } else {
      ...
      dispatch(pushMessage({ text: "使用優惠券失敗", status: 'failed' }))
      setCouponCode("");
    } finally {
      setIsLoading(false);
    }
  ...
  const handleCheckout = () => {
    if (!cart.carts || cart.carts.length === 0) {
      dispatch(pushMessage({ text: "購物車內沒有商品", status: 'failed' }))
    } else {
  ...
  return (
    <>
      <div className="container-fluid" style={{ paddingTop: "84px" }}>
